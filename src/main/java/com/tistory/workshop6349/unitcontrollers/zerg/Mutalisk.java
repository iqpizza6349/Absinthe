...
public class Mutalisk {
	...
	private static final List<AbsintheUnit> swarm = new ArrayList<>();
	public static void on_frame() {
		...
		for (AbsintheUnit muta : GameInfoCache.get_units(Alliance.SELF, Units.ZERG_MUTALISK)) {
			swarm.clear();
			if (UnitMovementManager.assignments.containsKey(muta.tag())) continue;
			for (AbsintheUnit muta2 : GameInfoCache.get_units(Alliance.SELF, Units.ZERG_MUTALISK)) {
		...
		if (swarm_target != null && swarm_center != null) {
			...
			for (AbsintheUnit mutaf: GameInfoCache.get_units(Alliance.SELF)) {
