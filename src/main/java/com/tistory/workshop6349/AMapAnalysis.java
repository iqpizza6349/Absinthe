...
public class AMapAnalysis {
    ...
    static {
        ...
        Point2d min = AGame.getGameInfo().getStartRaw().get().getPlayableArea().getP0().toPoint2d();
        Point2d max = AGame.getGameInfo().getStartRaw().get().getPlayableArea().getP1().toPoint2d();
        for (int x = (int) min.getX(); x < max.getX(); x ++) {
            for (int y = (int) min.getY(); y < max.getY(); y ++) {
                pathable[x][y] = AGame.pathAble(Point2d.of(x, y));
                height[x][y] = AGame.height(Point2d.of(x, y));
            }
        }
        for (int x = (int) min.getX(); x < max.getX(); x ++) {
            for (int y = (int) min.getY(); y < max.getY(); y ++) {
