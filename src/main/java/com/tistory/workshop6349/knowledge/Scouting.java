...
public class Scouting {
    ...
    public static AbsintheUnit scout = null;
    public static AbsintheUnit patrol_scout = null;
    ...
    public static void on_frame() {
        ...
        if (spawns.size() > 1) {
            for (AbsintheUnit unit: GameInfoCache.get_units(Alliance.ENEMY)) {
        ...
        if (!has_pulled_back && !Game.has_upgrade(Upgrades.OVERLORD_SPEED)) {
            if (Wisdom.proxy_detected() || Wisdom.all_in_detected()) {
                ...
                for (AbsintheUnit overlord: GameInfoCache.get_units(Alliance.SELF, Units.ZERG_OVERLORD)) {
    ...
    private static void assign_scout() {
        for (AbsintheUnit unit: GameInfoCache.get_units(Alliance.SELF, RaceInterface.get_race_worker())) {
    ...
    public static boolean is_scout(AbsintheUnit ally) {
