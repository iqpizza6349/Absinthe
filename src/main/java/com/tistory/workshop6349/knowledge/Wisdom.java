...
public class Wisdom {
    ...
    public static boolean cannon_rush() {
        for (AbsintheUnit u: GameInfoCache.get_units(Alliance.ENEMY, Units.PROTOSS_PHOTON_CANNON)) {
            if (u.distance(BaseManager.main_base().location) < u.distance(Scouting.closest_enemy_spawn())) {
                return true;
            }
        }
    ...
    public static boolean worker_rush() {
        ...
        if (Game.army_supply() < 10 && BaseManager.base_count() == 1) {
            outer: for (AbsintheUnit enemy: GameInfoCache.get_units(Alliance.ENEMY)) {
