...
import com.tistory.workshop6349.build.TechLevelManager.TechLevel;
...
public class Composition {
    ...
    private static final Map<TechLevel, Map<UnitType, Map<UnitType, Double>>> counters = new HashMap<>();
    private static final Map<UnitType, Integer> limits = new HashMap<>();

    private static final List<UnitType> units;
    private static final Set<UnitType> flying;
    ...
    static {
        ...
        for (TechLevel t: TechLevel.values()) {
    ...
    public static List<UnitType> filler_comp() {
        ...
        if ((EnemyModel.enemy_floated() || EnemyModel.enemyBaseCount() == 1) && Game.army_supply() > EnemyModel.enemyArmy() * 3 && BaseManager.base_count() >= 4) {
            return List.of(Units.ZERG_MUTALISK);
        }
        ...
        if (TechLevelManager.getTechLevel() == TechLevel.HATCH) {
            if (GameInfoCache.count(Units.ZERG_DRONE) < 40) {
                return List.of(Units.ZERG_ZERGLING);
            }
